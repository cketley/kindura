 ...
 * @author Andrew Woods
 */
public class DuraServiceClient {

    /*private static final String USERNAME = "jun";  // replace as necessary
    private static final String PASSWORD = "p1ssw2rd";   // replace as necessary
    private static final String HOST = "kindura.duracloud.org"; // replace as necessary
    */
    private static final String USERNAME = "root";  // replace as necessary
    private static final String PASSWORD = "rpw";   // replace as necessary
    ...
    //private static final String PORT = "443";      // replace as necessary
    private static final String CONTEXT = "duraservice";

    private ServicesManager servicesManager;

    public DuraServiceClient() {
    ...
    /**
     * This method executes the complete cycle of the image-transformer-service,
     * from deployment through undeployment.
     *
     * @param space over which service will be run
     * @throws Exception
     */
    public void runImageTransformerOverSpace(String oldSpace, String newSpace, String namePrefix, boolean bmpToJPG, boolean tiffToJPG) throws Exception {
        // Find all available services.
        List<ServiceInfo> services = getAvailableServices();

        // From available services, select out the image-transformer-service.
        ServiceInfo service = getImageTransformerService(services);

        // Get valid space options for the image-transformer-service.
        List<String> spaces = getSpaceOptions(service);
        
        System.out.println("space.size()"+spaces.size());
        Iterator<String> iterator = spaces.iterator();
        while (iterator.hasNext()) {
        	System.out.println(iterator.next());
        }
        
        // Validate that the target space is a valid option.
        //validateTargetSpace(newSpace, spaces);

        // Deploy the service.
        int deploymentId = deployImageTransformerService(service, oldSpace, newSpace, namePrefix, bmpToJPG, tiffToJPG);
    ...
    /**
     * This method finds the image-transformer-service from within the listing of
     * all available services.
     *
     * @param services listing of all available services
     * @return the bit-integrity-service or throw a runtime exception
     */
    private ServiceInfo getImageTransformerService(List<ServiceInfo> services) {
        for (ServiceInfo service : services) {
            String contentId = service.getContentId();
            if (contentId.startsWith("imageconversionservice")) {
            	System.out.println("- Image Transformer Service found");
                return service;
            }
        }
        throw new RuntimeException("Image Transformer Service NOT found!");
    }
    ...
    private void validateTargetSpace(String target, List<String> spaces) {
        ...
        if (null == target || !spaces.contains(target)) {
        	...
        	System.out.println("Target space "+target+" found.");
        }
    }
     ...
     * @throws Exception
     */
    private int deployImageTransformerService(ServiceInfo service, String oldSpace, String newSpace, String namePrefix, boolean bmpToJPG, boolean tiffToJPG)
        throws Exception {
        System.out.printf("%n- Deploying service%n");
        return servicesManager.deployService(service.getId(),
                                             service.getUserConfigVersion(),
                                             createImageTransformerDeploymentConfig(oldSpace, newSpace, namePrefix, bmpToJPG, tiffToJPG),
                                             null);
    }
     ...
     * @return
     */
    private List<UserConfigModeSet> createImageTransformerDeploymentConfig(String oldSpace, String newSpace, String namePrefix, boolean bmpToJPG, boolean tiffToJPG) {
        List<UserConfigModeSet> userConfigModeSets = new ArrayList<UserConfigModeSet>();
        ...
        configs.add(new TextUserConfig("storeId", "", "0"));
        //configs.add(new TextUserConfig("targetSpaceId", "", space));
        configs.add(new TextUserConfig("sourceSpaceId", "", oldSpace));
        configs.add(new TextUserConfig("destSpaceId", "", newSpace));
        configs.add(new TextUserConfig("toFormat", "", "jpg"));
        if (bmpToJPG == true) {
        	configs.add(new TextUserConfig("nameSuffix", "", "bmp"));
        }
        if (tiffToJPG == true) {
        	configs.add(new TextUserConfig("nameSuffix", "", "tiff"));
        }
        configs.add(new TextUserConfig("namePrefix", "",namePrefix));
        //configs.add();
    ...
    public static void main(String[] args) throws Exception {
        ...
        String oldSpace = "image1";
        
        String newSpace = "image2";
        
        String namePrefix = "gif";
        
        DuraServiceClient serviceClient = new DuraServiceClient();

        try {
            //serviceClient.runBitIntegrityOverSpace(space);
        	serviceClient.runImageTransformerOverSpace(oldSpace, newSpace, namePrefix, true, false);
        } catch (Exception e) {
